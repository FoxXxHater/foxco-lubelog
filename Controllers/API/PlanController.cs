    ...
    public partial class APIController
    {
        ...
        public IActionResult AddPlanRecord(int vehicleId, PlanRecordExportModel input)
        {
            ...
            try
            {
                ...
                _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromPlanRecord(planRecord, "planrecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeletePlanRecord(int id)
        {
            ...
            if (result)
            {
                _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromPlanRecord(existingRecord, "planrecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdatePlanRecord(PlanRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromPlanRecord(existingRecord, "planrecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
