    ...
    public partial class APIController
    {
        ...
        public IActionResult AddGasRecord(int vehicleId, GasRecordExportModel input)
        {
            ...
            try
            {
                ...
                _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromGasRecord(gasRecord, "gasrecord.add.api", User.Identity?.Name ?? string.Empty));
        ...
        public IActionResult DeleteGasRecord(int id)
        {
            ...
            if (result)
            {
                _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromGasRecord(existingRecord, "gasrecord.delete.api", User.Identity?.Name ?? string.Empty));
            }
        ...
        public IActionResult UpdateGasRecord(GasRecordExportModel input)
        {
            ...
            try
            {
                ...
                if (existingRecord != null && existingRecord.Id == int.Parse(input.Id))
                {
                    ...
                    _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromGasRecord(existingRecord, "gasrecord.update.api", User.Identity?.Name ?? string.Empty));
                }
                else
                {
                    Response.StatusCode = 400;
