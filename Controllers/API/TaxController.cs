...
namespace CarCareTracker.Controllers
{
    public partial class APIController
    {
        ...
        public IActionResult AllTaxRecords(MethodParameter parameters)
        {
            ...
            var result = vehicleRecords.Select(x => new TaxRecordExportModel { VehicleId = x.VehicleId.ToString(), Id = x.Id.ToString(), Date = x.Date.ToShortDateString(), Description = x.Description, Cost = x.Cost.ToString(), Notes = x.Notes, ExtraFields = x.ExtraFields, Files = x.Files, Tags = string.Join(' ', x.Tags) });
        ...
        public IActionResult AddTaxRecord(int vehicleId, TaxRecordExportModel input)
        {
            ...
            try
            {
                ...
                _eventLogic.PublishEvent(GetUserID(), WebHookPayload.FromTaxRecord(taxRecord, "taxrecord.add.api", User.Identity?.Name ?? string.Empty));
